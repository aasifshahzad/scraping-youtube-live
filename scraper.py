...
def parse_videos(videos):
  ...
  views_time_tag = driver.find_element(By.XPATH, views_time_xpath)
  views_time_text = views_time_tag.text.split()
  views = ' '.join(views_time_text[1:3])
  upload_time = ' '.join(views_time_text[3:6])
  video_discription_tag =    driver.find_element(By.XPATH, '//yt-formatted-string[@id="description-text" and @class="style-scope ytd-video-renderer" ]')
...
if __name__ == '__main__':
  ...
  # title, url, thumnail_url, channel, views, uploaded
    ...
    videos_data = [parse_videos(videos)for video in videos[:10]]
